...
export default function Register() {
    ...
    try {
      const res = await axios({
        ...
        data: {
          ...
          dob: moment(selectedDate).unix()?.toString(),
  ...
  const validationFormSchema = yup.object().shape({
    ...
    cities: yup.string().required("City name is required"),
    address: yup
      .string("Please enter valid address")
      .required("Address is required ")
      .min(10, "Please enter at least 10 characters")
      .max(350, "You can enter only 350 characters"),
    zipcode: yup.number("Enter valid ZipCode").required("ZipCode is required "),
  });
  const [selectedDate, setSelectedDate] = React.useState(new Date());
  const [checked, setChecked] = React.useState(true);
  const [agree, setAgree] = useState(false);
  console.log(
    "moment(selectedDate).unix()?.toString()",
    moment(selectedDate).unix()?.toString()
  );
